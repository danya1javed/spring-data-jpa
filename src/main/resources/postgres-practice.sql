...
-- my solution: best imo
 WITH RECURSIVE downRec AS (
  	select memid, recommendedby, firstname, surname from cd.members where recommendedby = 1
  	UNION
  	select omem.memid, omem.recommendedby, omem.firstname, omem.surname from cd.members omem
  	join downRec rmem on omem.recommendedby = rmem.memid
 ) select memid, firstname, surname from downRec order by memid;

-- my solution 2
 WITH RECURSIVE downRec AS (
  	select memid from cd.members where recommendedby = 1
  	UNION
  	select omem.memid from cd.members omem
  	join downRec rmem on omem.recommendedby = rmem.memid
 )
 select memid, firstname, surname from cd.members
 where memid in (select memid from downRec);
...
-- Paramiterized CTEs
 ...
 WITH RECURSIVE
 	upwardRec AS (
  		select memid, recommendedby, firstname, surname from cd.members
  		UNION ALL
 		select u.memid, rm.recommendedby, u.firstname, u.surname
 		from upwardRec rm inner join cd.members u on u.recommendedby = rm.memid
 ) select * from upwardRec
 where memid in (12, 22) and recommendedBy IS NOT NULL;
